...
for i in range(len(countries)):
    ...
    cur.execute("SELECT COUNT(CountryOfWinner) FROM Jobs WHERE CountryOfWinner = '" + country + "' AND ((Category = 4) OR (Category = 5))")
    num = cur.fetchone()[0]

    cur.execute("SELECT COUNT(CountryOfWinner) FROM ReviewJobs WHERE CountryOfWinner = '" + country + "' AND ((Category = 4) OR (Category = 5))")
    num += cur.fetchone()[0]
